...
myDataPlot <- function (maintitle = "", xlabel = "Plasma [11-KT] (ng/mL)", cutofflabel = "Minijack cutoff", cutoffunits = "ng/mL"){
  xValues <- seq(myFitData$xfitLower, myFitData$xfitUpper, length=200)
  ...
  yvalues1 <- yfit1*diff(myFitData$h$mids[1:2])*length(mydata$data)
  yvalues2 <- yfit2*diff(myFitData$h$mids[1:2])*length(mydata$data)

  v1 = seq(myFitData$v1.lower, myFitData$v1.upper,length=11)
  ...
  hist(mydata$data, breaks = 15, density = 10, col = "grey", xaxt="n", xlab = xlabel,
       ylim = c(0, myFitData$ylim.upper), xlim = c(myFitData$xlim.lower, myFitData$xlim.upper), main = maintitle,
  ...
  text(cutoff+(myFitData$step/2), myFitData$ylim.upper-5, adj = c(0, 1), paste(cutofflabel, "\n =",
