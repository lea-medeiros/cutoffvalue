...
The functions in this packaged are written to utilize the example data set (located at "R/exampledata.xlsx") and will use it by default if the path for a dataset is not provided. The examples below specify this, along with any other default parameters for each function.
...
> The `modes` function uses the example dataset (located at "R/exampledata.xlsx") by default. Thus, if the path for the dataset is not specified (e.g., running modes() in the console), this is the data that will be used.
...
> The `cutoffplot` function uses the example dataset (located at "R/exampledata.xlsx") by default. Thus, if the path for the dataset is not specified (e.g., running cutoffplot() in the console), this is the data that will be used. Parameters (and their defaults) for the resulting graph are listed below.

> When given a label (i.e., "plottyMcPlotty" in the example below), this function returns the log-transformed cutoff value to the Environment. 
...
## Use the `importdata` function to import your raw dataset

This function is used to import your dataset into the R Studio Environment. It uses the readxl::read_excel() function to import data, and assumes it's a file containing a single column of log- or ln-transformed data. After importing the dataset, the function removes an blank rows and transforms the data into a list of numbers. This list of values (`mydata$data`) is returned to the Environment along with the minimum (`mydata$lower`) and maximum values (`mydata$upper`), which are used in subsequent functions. **A label must be applied to this function for it to be used in subsequent functions.**

> If nothing is specified (i.e., only "importdata()" is typed into the console), this function will use "R/exampledata.xlsx" as the default dataset. 

```{r importdata}
rawdata <- importdata("R/exampledata.xlsx")
```
...
> The `datamodel` function uses the example dataset (located at "R/exampledata.xlsx") by default. Thus, if the path for the dataset is not specified (e.g., running importdata() in the console), this is the dataset that will be used.
...
> The defaults for this function are "R/exampledata.xlsx" as the raw dataset and 0.5 as the probability (i.e., running "findcutoff()" in the console will use these values)
...
> If nothing is specified (i.e., only "fitparams()" is typed into the console), this function will use "R/exampledata.xlsx" as the default dataset. 
...
> If nothing is specified (i.e., only "curves()" is typed into the console), this function will use "R/exampledata.xlsx" as the default dataset. 
