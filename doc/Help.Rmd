...
The functions in this package are written to utilize the example data set (an internal dataset object identified as "cutoffvalue:::exampledata") and will use it by default if the path for a dataset is not provided. The examples below specify this, along with any other default parameters for each function.
...
plottyMcplotty <- cutoffplot(cutoffvalue:::exampledata, title, xlab, cutofflab, cutoffunits, LowerMode_col, LowerMode_lty, LowerMode_lwd, UpperMode_col, UpperMode_lty, UpperMode_lwd, cutoffvalue_col, cutoffvalue_lty, cutoffvalue_lwd)
```
...
> The defaults for this function are an internal dataset object identified as "cutoffvalue:::exampledata" as the raw dataset and 0.5 as the probability (i.e., running "findcutoff()" in the console will use these values)
